...
TEST(BaseLinkPublisher, orientationToOrientationAngleWithItself) {
	...
	// angle with itself = 0?
	EXPECT_FLOAT_EQ(0, kml::orientationToOrientationAngle(rot1, rot1));
	EXPECT_FLOAT_EQ(0, kml::orientationToOrientationAngle(rot2, rot2));
}
...
TEST(BaseLinkPublisher, orientationToOrientationAngleOrderDoesNotMatter) {
	...
	EXPECT_FLOAT_EQ(180, kml::orientationToOrientationAngle(rot1, rot2));
	EXPECT_FLOAT_EQ(180, kml::orientationToOrientationAngle(rot2, rot1));

	rot2.setRPY(0, 0, M_PI * 1.5);

	// angle = 90°, order of orientation does not matter?
	EXPECT_FLOAT_EQ(90, kml::orientationToOrientationAngle(rot1, rot2));
	EXPECT_FLOAT_EQ(90, kml::orientationToOrientationAngle(rot2, rot1));
}
...
TEST(BaseLinkPublisher, poseToPoseAbsDistance) {
	...
	EXPECT_FLOAT_EQ(1, kml::poseToPoseAbsDistance(p1, p2));
	EXPECT_FLOAT_EQ(1, kml::poseToPoseAbsDistance(p2, p1));

	p2.setOrigin(tf::Vector3(4, 3, 13));

	EXPECT_FLOAT_EQ(5, kml::poseToPoseAbsDistance(p1, p2));
	EXPECT_FLOAT_EQ(5, kml::poseToPoseAbsDistance(p2, p1));
}
